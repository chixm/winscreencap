...
// Screen Capture using Windows Direct3D11CaptureFramePool
...
import (
	...
	"github.com/chixm/winscreencap/d3d"
...
func (c *CaptureHandler) StartCapture(hwnd win.HWND) error {
	...
	go func() {
		...
		// Set frame settings
		var eventObject = d3d.NewDirect3D11CaptureFramePool(c.onFrameArrived)
...
func (c *CaptureHandler) onFrameArrived(this_ *uintptr, sender *winrt.IDirect3D11CaptureFramePool, args *ole.IInspectable) uintptr {
	_ = (*d3d.Direct3D11CaptureFramePool)(unsafe.Pointer(this_))
	...
	// Do conversions here
	log.Println(frame.VTable().Surface)
	var desc d3d.D3D11_TEXTURE2D_DESC
	texture := (*d3d.ID3D11Texture2D)(unsafe.Pointer(frame.VTable().Surface))
